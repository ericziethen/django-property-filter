...
def test_lookup_xpr(fixture_property_time_range_filter, lookup_xpr, lookup_val, result_list):
    ...
    filter_fs = TimeRangeFilterSet({'time_after': lookup_val[0], 'time_before': lookup_val[1]}, queryset=TimeClass.objects.all())
    ...
    prop_filter_fs = PropertyTimeRangeFilterSet({'prop_time_after': lookup_val[0], 'prop_time_before': lookup_val[1]}, queryset=TimeClass.objects.all())
    ...
    implicit_filter_fs = ImplicitFilterSet({'prop_time__range_after': lookup_val[0], 'prop_time__range_before': lookup_val[1]}, queryset=TimeClass.objects.all())
