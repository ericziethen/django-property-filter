...
def test_lookup_xpr(fixture_property_date_time_filter, lookup_xpr, lookup_val, result_list):
    ...
    filter_fs_mixed = DateTimeFilterSet({'date_time': lookup_val}, queryset=DateTimeFilterModel.objects.all())
    prop_filter_fs_mixed = PropertyDateTimeFilterSet({'prop_date_time': lookup_val}, queryset=DateTimeFilterModel.objects.all())
