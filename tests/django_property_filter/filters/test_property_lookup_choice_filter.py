...
def test_lookup_xpr(fixture_property_lookup_choice_filter, lookup_xpr, lookup_val, result_list):
    ...
    class LookupChoiceFilterSet(FilterSet):
        number = LookupChoiceFilter(field_name='number', lookup_expr=lookup_xpr)

        class Meta:
            model = LookupChoiceFilterModel
            fields = ['number']

    filter_fs = LookupChoiceFilterSet({'number': lookup_val}, queryset=LookupChoiceFilterModel.objects.all())
    assert set(filter_fs.qs.values_list('id', flat=True)) == set(result_list)

    # Compare with Explicit Filter using a normal Filterset
    class PropertyLookupChoiceFilterSet(FilterSet):
        prop_number = PropertyLookupChoiceFilter(field_name='prop_number', lookup_expr=lookup_xpr)

        class Meta:
            model = LookupChoiceFilterModel
            fields = ['prop_number']

    prop_filter_fs = PropertyLookupChoiceFilterSet({'prop_number': lookup_val}, queryset=LookupChoiceFilterModel.objects.all())
    ...
    class ImplicitFilterSet(PropertyFilterSet):
        ...
        class Meta:
            model = LookupChoiceFilterModel
            exclude = ['number']
            property_fields = [('prop_number', PropertyLookupChoiceFilter, [lookup_xpr])]

    implicit_filter_fs = ImplicitFilterSet({F'prop_number__{lookup_xpr}': lookup_val}, queryset=LookupChoiceFilterModel.objects.all())
